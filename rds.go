...
func (builder RDSBuilder) Populate(session *session.Session, tree *AWSTree) {
	...
	data := make([]*rds.DBInstance, 0, 50)
	remaining := true
	marker := ""

	//If MaxRecords has been reached, a token called marker will be used to retrieve the remaining results
	for remaining {
		var input rds.DescribeDBInstancesInput

		if len(marker) > 0 {
			input = rds.DescribeDBInstancesInput{Marker: &marker, MaxRecords: &maxRecords}
		} else {
		...
		data = append(data, output.DBInstances...)
	...
	rdsData := RDSData{Databases: make([]DBInstances, 0, len(data))}
	for _, db := range data {
...
func buildInstance(svc *rds.RDS, db *rds.DBInstance, svcr *ec2.EC2, service *cloudtrail.CloudTrail) *DBInstances {
	...
	//To assign the details of users accesing database into struct RDSUser
	for _, iamuser := range buildIAMUser(service, d) {
		...
		if strings.Contains(str, substr) {
			...
			d.Users = append(d.Users, userIAM)
		} else {
	...
	for _, snapshot := range buildSnapshot(svc, d) {
...
func buildSnapshot(svc *rds.RDS, db *DBInstances) []*rds.DBSnapshot {
	maxRecords := int64(100)
	ss := make([]*rds.DBSnapshot, 0, 100)
	remaining := true
	marker := ""
	...
	for remaining {
		...
		ss = append(ss, output.DBSnapshots...)
		if output.Marker != nil {
			marker = *output.Marker
		} else {
			remaining = false
...
func buildIAMUser(service *cloudtrail.CloudTrail, db *DBInstances) []*cloudtrail.Event {
	maxResults := int64(1)
	users := make([]*cloudtrail.Event, 0, 100)
	remaining := true
	nextToken := ""
	...
	for remaining {
		...
		output, err := service.LookupEvents(&input)
		if err != nil {
			log.Fatalf("Couldn't list users, please enable CloudTrail service", err)
		}

		users = append(users, output.Events...)
